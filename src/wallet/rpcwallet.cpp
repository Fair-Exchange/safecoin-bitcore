...
void WalletTxToJSON(const CWalletTx& wtx, Object& entry)
{
    ...
    //n = wtx.vout.size();
    //for (i=0; i<n; i++)
    //    interest += safecoin_interest(mapBlockIndex[wtx.hashBlock]->nHeight,wtx.vout[i].nValue,wtx.nLockTime,mapBlockIndex[wtx.hashBlock]->nTime);
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Safecoin address: ")+s.name_);
...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs) {
        ...
        if ( out.tx->nLockTime != 0 )
        {
            ...
            safecoin_accrued_interest(&txheight,&locktime,out.tx->GetHash(),out.i,0,nValue);
