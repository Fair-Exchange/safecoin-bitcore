...
//fprintf(stderr,"ht.%d vout0 t%u\n",pindex->nHeight,tx.nLockTime);
	...
	//sc  safecoin_earned_interest(pindex->nHeight,sum);
        CTxUndo undoDummy;
        if (i > 0) {
...
static bool ActivateBestChainStep(CValidationState &state, CBlockIndex *pindexMostWork, CBlock *pblock) {
    ...
    if ( SAFECOIN_REWIND != 0 && chainActive.Tip()->nHeight > SAFECOIN_REWIND )
    {
        static int32_t didinit;
        if ( didinit++ == 0 )
        {
            while (chainActive.Tip()->nHeight > SAFECOIN_REWIND )
            {
                ...
                bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if ( safecoin_check_deposit(height,block) < 0 )
        return(false);
    return true;
}
