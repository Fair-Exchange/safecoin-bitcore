...
void safecoin_userpass(char *username,char *password,FILE *fp)
{
...
int32_t safecoin_userpass(char *userpass,char *symbol)
{
...
uint32_t safecoin_assetmagic(char *symbol,uint64_t supply)
{
...
/*int32_t safecoin_shortflag(char *symbol)
{
...
uint16_t safecoin_assetport(uint32_t magic)
{
    if ( magic == 0x8de4eef9 )
        return(8770);
...
uint16_t safecoin_port(char *symbol,uint64_t supply,uint32_t *magicp)
{
    if ( symbol == 0 || symbol[0] == 0 || strcmp("SAFE",symbol) == 0 )
    {
        *magicp = 0x8de4eef9;
    ...
    *magicp = safecoin_assetmagic(symbol,supply);
    return(safecoin_assetport(*magicp));
}
...
int32_t safecoin_whoami(char *pubkeystr,int32_t height)
{
    ...
    pubkeystr[66] = 0;
    safecoin_chosennotary(&notaryid,height,NOTARY_PUBKEY33);
    return(notaryid);
}
...
void safecoin_args()
{
    ...
    if ( name.c_str()[0] != 0 )
    {
        ...
        ASSETCHAINS_PORT = safecoin_port(ASSETCHAINS_SYMBOL,ASSETCHAINS_SUPPLY,&ASSETCHAINS_MAGIC);
