...
extern int32_t ASSETCHAINS_SEED,IS_SAFECOIN_NOTARY,USE_EXTERNAL_PUBKEY,SAFECOIN_CHOSEN_ONE,ASSETCHAIN_INIT,SAFECOIN_INITDONE,SAFECOIN_ON_DEMAND,SAFECOIN_INITDONE;
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    if ( ASSETCHAINS_SYMBOL[0] != 0 && chainActive.Tip()->nHeight >= ASSETCHAINS_MINHEIGHT )
    {
        ...
        {
            deposits = safecoin_paxtotal();
            if ( SAFECOIN_INITDONE == 0 || (safecoin_baseid(ASSETCHAINS_SYMBOL) >= 0 && (isrealtime= safecoin_isrealtime(&safeheight)) == 0) )
            {
...

    {
        ...
        if ( ASSETCHAINS_SYMBOL[0] == 0 )
        {
            ...
            {
                if ( (opretlen= safecoin_pax_opreturn(opret,sizeof(opret))) > 0 ) // have pricefeed
                {
                    txNew.vout.resize(2);
...
// Internal miner
//
int8_t safecoin_minerid(int32_t height);
...
CBlockTemplate* CreateNewBlockWithKey(CReserveKey& reservekey)
{
    ...
    {
        for (i=0; i<65; i++)
            fprintf(stderr,"%d ",safecoin_minerid(chainActive.Tip()->nHeight-i));
        fprintf(stderr," minerids.special %d from ht.%d\n",safecoin_is_special(chainActive.Tip()->nHeight+1,NOTARY_PUBKEY33),chainActive.Tip()->nHeight);
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    safecoin_chosennotary(&notaryid,chainActive.Tip()->nHeight,NOTARY_PUBKEY33);
    ...
    try {
        ...
        while (true)
        {
            ...
            if ( ASSETCHAINS_SYMBOL[0] == 0 && safecoin_is_special(pindexPrev->nHeight+1,NOTARY_PUBKEY33) > 0 )
            {
                if ( (Mining_height % SAFECOIN_ELECTION_GAP) > 64 || (Mining_height % SAFECOIN_ELECTION_GAP) == 0 )
                {
