...
bool CBlockTreeDB::ReadAddressUnspentIndex(uint160 addressHash, int type,
    ...
    while (pcursor->Valid()) {
        ...
        try {
            ...
            ssKey >> auk;
            if (chType == DB_ADDRESSUNSPENTINDEX && auk.hashBytes == addressHash) {
                try {
                    leveldb::Slice slValue = pcursor->value();
                    CDataStream ssValue(slValue.data(), slValue.data()+slValue.size(), SER_DISK, CLIENT_VERSION);
                    CAddressUnspentValue nValue;
                    ssValue >> nValue;
                    unspentOutputs.push_back(make_pair(auk, nValue));
                    pcursor->Next();
                } catch (const std::exception& e) {
...
bool CBlockTreeDB::ReadAddressIndex(uint160 addressHash, int type,
    ...
    while (pcursor->Valid()) {
        ...
        try {
            ...
            ssKey >> aik;
            if (chType == DB_ADDRESSINDEX && aik.hashBytes == addressHash) {
                ...
                try {
                    ...
                    CAmount nValue;
                    ssValue >> nValue;
                    addressIndex.push_back(make_pair(aik, nValue));
                    pcursor->Next();
                } catch (const std::exception& e) {
...
bool CBlockTreeDB::ReadTimestampIndex(const unsigned int &high, const unsigned int &low, const bool fActiveOnly, std::vector<std::pair<uint256, unsigned int> > &hashes) {
    ...
    while (pcursor->Valid()) {
        ...
        try {
            ...
            char chType;
            ssKey >> chType;
            CTimestampIndexKey tik;
            ssKey >> tik;
            if (chType == DB_TIMESTAMPINDEX && tik.timestamp < high) {
                if (fActiveOnly) {
                    if (blockOnchainActive(tik.blockHash)) {
                        hashes.push_back(std::make_pair(tik.blockHash, tik.timestamp));
                    }
                } else {
                    hashes.push_back(std::make_pair(tik.blockHash, tik.timestamp));
                }
