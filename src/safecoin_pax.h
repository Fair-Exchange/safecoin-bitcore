...
int32_t dpow_readprices(uint8_t *data,uint32_t *timestampp,double *SAFEBTCp,double *BTCUSDp,double *CNYUSDp,uint32_t *pvals)
{
...
int32_t safecoin_pax_opreturn(uint8_t *opret,int32_t maxsize)
{
        ...
        if ( fsize <= maxsize-4 && fsize <= sizeof(data) && fsize > sizeof(crc32) )
        {
            if ( (retval= (int32_t)fread(data,1,fsize,fp)) == fsize )
            {
                ...
                if ( check == crc32 )
                {
                    ...
                    if ( dpow_readprices(&data[len],&timestamp,&SAFEBTC,&BTCUSD,&CNYUSD,pvals) > 0 )
                    {
                        if ( 0 && lastcrc != crc32 )
                        {
                            for (i=0; i<32; i++)
...
uint64_t safecoin_paxcalc(uint32_t *pvals,int32_t baseid,int32_t relid,uint64_t basevolume,uint64_t safebtc,uint64_t btcusd)
{
    uint32_t pvalb,pvalr; uint64_t usdvol,baseusd,usdsafe,baserel,ranked[32];
    ...
    if ( (pvalb= pvals[baseid]) != 0 )
    {
        if ( relid == MAX_CURRENCIES )
        {
            ...
            if ( pvals[USD] != 0 && safebtc != 0 && btcusd != 0 )
            {
                ...
                //printf("safebtc.%llu btcusd.%llu base -> USD %llu, usdsafe %llu usdvol %llu -> %llu\n",(long long)safebtc,(long long)btcusd,(long long)baseusd,(long long)usdsafe,(long long)usdvol,(long long)(MINDENOMS[USD] * safecoin_paxvol(usdvol,usdsafe) / (MINDENOMS[baseid]/100)));
                //printf("usdsafe.%llu basevolume.%llu baseusd.%llu paxvol.%llu usdvol.%llu -> %.8f\n",(long long)usdsafe,(long long)basevolume,(long long)baseusd,(long long)safecoin_paxvol(basevolume,baseusd),(long long)usdvol,dstr(MINDENOMS[USD] * safecoin_paxvol(usdvol,usdsafe)));
                return(MINDENOMS[USD] * safecoin_paxvol(usdvol,usdsafe) / (MINDENOMS[baseid]/100));
    ...
    if ( (baseid= safecoin_baseid(base)) >= 0 && (relid= safecoin_baseid(rel)) >= 0 )
    {
        ...
        for (i=NUM_PRICES-1; i>=0; i--)
        {
            ...
            if ( *ptr < height )
            {
                ...
                {
                    *safebtcp = ptr[MAX_CURRENCIES + 1] / 539;
                    *btcusdp = ptr[MAX_CURRENCIES + 2] / 539;
                }
                //portable_mutex_unlock(&safecoin_mutex);
                if ( safebtc != 0 && btcusd != 0 )
                    return(safecoin_paxcalc(&ptr[1],baseid,relid,basevolume,safebtc,btcusd));
                else return(0);
            }
        }
        //portable_mutex_unlock(&safecoin_mutex);
...
uint64_t safecoin_paxpriceB(uint64_t *seedp,int32_t height,char *base,char *rel,uint64_t basevolume)
{
    ...
    *seedp = safecoin_seed(height);
    ...
    //if ( safecoin_safebtcusd(0,&safebtc,&btcusd,height) < 0 ) crashes when via passthru GUI use
    {
        ...
        safebtc = safecoin_paxcorrelation(safebtcs,numvotes,*seedp) * 539;
        btcusd = safecoin_paxcorrelation(btcusds,numvotes,*seedp) * 539;
    ...
    if ( nonz <= (numvotes >> 1) )
    {
        //printf("safebtc %llu btcusd %llu\n",(long long)safebtc,(long long)btcusd);
        //printf("safecoin_paxprice nonz.%d of numvotes.%d\n",nonz,numvotes);
        return(0);
    }
    return(safecoin_paxcorrelation(votes,numvotes,*seedp) * basevolume / 100000);
}
...
uint64_t safecoin_paxprice(uint64_t *seedp,int32_t height,char *base,char *rel,uint64_t basevolume)
{
    ...
    for (i=0; i<17; i++)
    {
        if ( (price= safecoin_paxpriceB(&seed,height-i,base,rel,basevolume)) != 0 )
        {
            sum += price;
            nonz++;
            if ( 0 && i == 1 && nonz == 2 )
...
int32_t safecoin_paxprices(int32_t *heights,uint64_t *prices,int32_t max,char *base,char *rel)
{
    ...
    if ( (baseid= safecoin_baseid(base)) >= 0 && (relid= safecoin_baseid(rel)) >= 0 )
    {
        for (i=NUM_PRICES-1; i>=0; i--)
        {
            ptr = &PVALS[36 * i];
            heights[num] = *ptr;
            prices[num] = safecoin_paxcalc(&ptr[1],baseid,relid,COIN,0,0);
            num++;
            if ( num >= max )
                return(num);
        }
...
void safecoin_paxpricefeed(int32_t height,uint8_t *pricefeed,int32_t opretlen)
{
    ...
    numpvals = dpow_readprices(pricefeed,&timestamp,&SAFEBTC,&BTCUSD,&CNYUSD,pvals);
    memset(&zero,0,sizeof(zero));
    safecoin_stateupdate(height,0,0,0,zero,0,0,pvals,numpvals,0,0,0,0,0,0);
    //printf("safecoin_paxpricefeed vout OP_RETURN.%d prices numpvals.%d opretlen.%d\n",height,numpvals,opretlen);
}
