...
Value minerids(const Array& params, bool fHelp)
{
    ...
    if ( (n= safecoin_minerids(minerids,height,(int32_t)(sizeof(minerids)/sizeof(*minerids)))) > 0 )
    {
        ...
        if ( numnotaries > 0 )
        {
            ...
            for (i=0; i<64; i++)
            {
                Object item; std::string hex,safeaddress; char *hexstr,safeaddr[64],*ptr; int32_t m;
                hex.resize(66);
...
{
    Array a; Object ret; int32_t i,j,n,m; char *hexstr;  uint8_t pubkeys[64][33]; char btcaddr[64],safeaddr[64],*ptr;
    if ( fHelp || params.size() != 1 )
