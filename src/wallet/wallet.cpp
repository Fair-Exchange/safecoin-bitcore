            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                    ...
                    (!coinControl || !coinControl->HasSelected() || coinControl->IsSelected((*it).first, i)))
                {
                    ...
                    if ( ASSETCHAINS_SYMBOL[0] == 0 && chainActive.Tip() != 0 && chainActive.Tip()->nHeight >= 60000 )
                    {
                        ...
                        {
                            safecoin_accrued_interest(&txheight,&locktime,wtxid,i,0,pcoin->vout[i].nValue);
                            if ( (tipindex= chainActive.Tip()) != 0 )
                            {
                                interest = safecoin_interest(txheight,pcoin->vout[i].nValue,locktime,tipindex->nTime);
                            } else interest = 0;
                            //interest = safecoin_interest(chainActive.Tip()->nHeight+1,pcoin->vout[i].nValue,pcoin->nLockTime,chainActive.Tip()->nTime);
                            if ( interest != 0 )
                            {
